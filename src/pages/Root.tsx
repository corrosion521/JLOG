...
function Root() {
  ...
  // 좌표를 담을 배열 상태 초기화
  const [latitudeArray, setLatitudeArray] = useState<number[]>([]); //위도
  const [longitudeArray, setLongitudeArray] = useState<number[]>([]); //경도
  const [timeArray, setTimeArray] = useState<string[]>([]); //시간
  ...
  //시간기록
  const [stimer, setStimer] = useState(''); //저장 내용1
  const [ftimer, setFtimer] = useState(''); //저장 내용2
  ...
  const currentStimer = () => {
    ...
    setStimer(`${format}T${hours}:${minutes}:${seconds}`);
  };
  const currentFtimer = () => {
    var date = new Date();
    var year = date.getFullYear();
    ...
    setFtimer(`${format}T${hours}:${minutes}:${seconds}`);
  };
  const currentTimeArray = () => {
    ...
    const minutes = String(date.getMinutes()).padStart(2, '0');
    const seconds = String(date.getSeconds()).padStart(2, '0');
    return `${format}T${hours}:${minutes}:${seconds}`;
  };
  ...
  const onRecordWriteList = () => {
    ...
    // 위의 과정을 반복문으로 처리할 경우
    let dataList = [];
    // 예를 들어 latitudeArray, longitudeArray, timeArray가 동일한 길이의 배열이라고 가정하면
    // for 문을 사용하여 배열을 순회하며 dataList에 데이터를 추가할 수 있습니다.
    for (let i = 0; i < latitudeArray.length; i++) {
      dataList.push({
        latitude: latitudeArray[i],
        longitude: longitudeArray[i],
        localDateTime: timeArray[i],
      });
    }
    //이동
    fetch('http://jlog.shop/location/save', {
      method: 'POST',
      headers: {
      ...
      .then(result => {
        ...
        if (result.status === 'OK') {
          ...
          // 실패 시 에러 메세지
          return Alert.alert('실패', result.message);
        }
      });
  };
  ...
  return (
    <View
      ...
      {recwrite ? (
        ...
        <Image
          source={require('../assets/powerOn.png')}
          style={styles.infoProfile}
        />
      ) : (
        <Image
          source={require('../assets/powerOff.png')}
          style={styles.infoProfile}
        />
      )}
      {/*테스트용 geolocation.  */}
      ...
      <View>
        <View
          ...
          {recwrite ? null : rec ? (
            <Text style={styles.infoEditText}>경로 기록 중</Text>
          ) : (
          ...
          {recwrite ? (
            <Text style={styles.infoEditText}>기록의 제목을 입력하세요</Text>
          ) : null}
        ...
        {recwrite ? null : timeFinished ? (
          <Pressable style={styles.button} onPress={onRecordWrite}>
            <Text style={styles.buttonText}>기록 저장</Text>
          </Pressable>
        ) : null}
        {timeFinished ? <View style={{marginTop: 10}}></View> : null}
        {recwrite ? null : rec ? (
          ...
          <Pressable style={styles.button} onPress={onRecordOff}>
            <Text style={styles.buttonText}>기록 종료</Text>
          </Pressable>
        ) : (
          // {/* // </View> */}

          <Pressable style={styles.button} onPress={onRecordOn}>
        ...
        {recwrite ? (
          <View style={{}}>
            <View
              ...
              <Pressable style={styles.button} onPress={onRecordWriteList}>
                <Text style={styles.buttonText}>저장 하기</Text>
              </Pressable>
              <Pressable style={styles.button} onPress={onRecordWriteOff}>
